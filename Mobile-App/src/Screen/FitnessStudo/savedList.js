...
import styles from './../AddToFavourite/favourite_styles'
import ReadMore from 'react-native-read-more-text';
import Modal from 'react-native-modal';
import { requestGetApi, getmypostlist } from '../../NetworkCall/Service';
import { getAsyncStorage } from '../../Routes/AsynstorageClass';
import { requestPostApiMedia, likePost, unlikePost } from '../../NetworkCall/Service';
import { requestDeleteApiMedia, unsavefrommypost, post_unfavourite } from '../../NetworkCall/Service';
...
class savedList extends React.Component {
    ...
    getUploadPostList = async () => {
        ...
        const { responseJson, err } = await requestGetApi(getmypostlist, body, 'GET', token_value)
    ...
    deletePost = async () => {
        ...
        const { responseJson, err } = await requestPostApiMedia(unsavefrommypost, formData, 'POST', token);
        if (responseJson.status == true) {
    ...
    RemovefromFavourite = async (postId, index) => {
        let token = await getAsyncStorage('tokenkey');
        const formData = new FormData();
        formData.append('my_post_id', postId);
    ...
    render() {
        ...
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: '#15141A' }} >
                <HeaderScreen title='Saved' navigation={this.props.navigation} />
                                ...
                                <View style={{ flexDirection: 'row', marginTop: "5%", marginBottom: "5%", marginLeft: '4%' }}>
                                    ...
                                    <View style={{ position: 'absolute', right: "10%" }}>
                                        <TouchableOpacity style={styles.fav_icon}
                                            ...
                                            onPress={() =>
                                                this.RemovefromFavourite(item.my_post_id, index)
                                            }>
                                            <Image
                                                style={styles.febImg2}

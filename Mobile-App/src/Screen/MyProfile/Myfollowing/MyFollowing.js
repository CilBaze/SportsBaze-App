...
class MyFollowing extends React.Component {
    ...
    getPublicPostList = async () => {
        ...
        const { responseJson, err } = await requestGetApi(getpublic_folloing, body, 'GET', token_value)
        console.log("My followeing list", responseJson)
        this.setState({ loading: false })
        if (responseJson.status == true) {
            PostData_arr = responseJson.data.records;
            if (this.state.PageNo == 1) {
                console.log("sjkdhkhd")
    ...
    render() {
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: '#15141A' }}>
                <HeaderScreen title='My Following' navigation={this.props.navigation} />

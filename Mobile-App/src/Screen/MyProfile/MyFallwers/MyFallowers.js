...
let subsports_name = ''
let myUserId = ''
class MyFallowers extends React.Component {
    ...
    componentDidMount = async () => {
        myUserId = await getAsyncStorage("myuserId");
    ...
    getPublicPostList = async () => {
        ...
        const { responseJson, err } = await requestGetApi(getpublic_follower, body, 'GET', token_value)
        console.log("My follower list", responseJson)
    ...
    Unfollowers = async () => {
        ...
        let token = await getAsyncStorage('tokenkey');
        console.log("token", token)
        console.log("user_id", this.state.isslectedId)
        var details = {
            'user_id': this.state.isslectedId
        };
        var formBody = [];
        for (var property in details) {
            var encodedKey = encodeURIComponent(property);
    ...
    toggleModal(user_id) {
        this.setState({
            isModalVisible: !this.state.isModalVisible, isslectedId: user_id
        });
    };
    NoClick() {
    ...
    render() {
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: '#15141A' }}>
                ...
                <HeaderScreen title='My Followers' navigation={this.props.navigation} />
                    ...
                    <FlatList data={this.state.PostData_list}
                        renderItem={({ item, index }) =>
                            <View style={styles.fieldBG}>
                                ...
                                {item.isFollowing == 0 ?
                                    <TouchableOpacity style={styles.followingBg} onPress={() => this.followers(item.user_id, item.isFollowing)} >

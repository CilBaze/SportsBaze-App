...
$(document).on('submit',"#postForm",function(e) {
    ...
    var formdata = { post_id: $(this).find('input[name=post_id]').val(), latitude: $(this).find('input[name=latitude]').val(),
     longitude: $(this).find('input[name=longitude]').val(), title: $(this).find('input[name=title]').val(), location: $(this).find('input[name=location]').val(), description: $(this).find('textarea[name=description]').val(), website_url: $(this).find('input[name=website_url]').val(), postal_code: $(this).find('input[name=postal_code]').val(),rating: $(this).find('input[name=rating]').val(),post_type: $(this).find('select[name=post_type]').val()};
    
    $.ajax({
        url: base_url + "admin/save_upload_video/sports_center",
        data: formdata,
        type: 'POST',
        dataType: 'json',
        beforeSend: function() {
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.status == 1) {
               ...
               $('#postForm').find('input[name=website_url]').val(post_details.website_url);
               ...
               $('#postForm').find('input[name=rating]').val(post_details.rating);
               $('#postForm').find('select[name=post_type]').val(post_details.post_type);

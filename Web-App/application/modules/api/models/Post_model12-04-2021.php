...
class Post_model extends MY_Model{
    ...
    public function getAllPosts($condition, $offest = 0, $limit = 20){
        ...
        $this->db->select('p.id as post_id, p.description, p.title, p.website_url, p.location, p.latitude, p.longitude, p.views, p.link_contact_form, p.contact_email, p.status, p.shared_post_id, p.created_date, p.created_by as user_id, p.sports_id, p.sub_sports_id, sp.name as sport_name, ssp.name as sub_sports_name, u.first_name as user_first_name, u.last_name as user_last_name, u.email, u.user_group, (CASE WHEN u.user_group = "1" THEN "Admin" WHEN u.user_group = "2" THEN "Fan / User" WHEN u.user_group = "3" THEN "Coach / Trainer" WHEN u.user_group = "4" THEN "Scout / Club" WHEN u.user_group = "5" THEN "Athlete" ELSE "Admin" end ) as user_group_name, CASE WHEN u.profile_pic IS NOT NULL AND u.profile_pic != "" THEN CONCAT("'.base_url('uploads/profile/').'", u.id, "/", u.profile_pic) ELSE "" END as profile_pic, (SELECT count(*) FROM '.Tables::POST_FAVOURITE.' WHERE post_id=p.id) as total_favourite '.$distance);
        ...
        if(isset($condition['sports_id']) && !empty($condition['sports_id']))
            $this->db->where('p.sports_id', $condition['sports_id']);
    ...
    public function getUserFavourite($condition, $offest = 0, $limit = 20){
        $this->db->select('p.id as post_id, p.description, p.title, p.website_url, p.location, p.latitude, p.longitude, p.views, p.link_contact_form, p.contact_email, p.status, p.created_date, p.created_by as user_id, p.sports_id, p.sub_sports_id, sp.name as sport_name, ssp.name as sub_sports_name, u.first_name as user_first_name, u.last_name as user_last_name, u.email, u.user_group, CASE WHEN u.profile_pic IS NOT NULL AND u.profile_pic != "" THEN CONCAT("'.base_url('uploads/profile/').'", u.id, "/", u.profile_pic) ELSE "" END as profile_pic, (CASE WHEN u.user_group = "1" THEN "Admin" WHEN u.user_group = "2" THEN "Fan / User" WHEN u.user_group = "3" THEN "Coach / Trainer" WHEN u.user_group = "4" THEN "Scout / Club" WHEN u.user_group = "5" THEN "Athlete" ELSE "Admin" end ) as user_group_name, (SELECT count(*) FROM '.Tables::POST_FAVOURITE.' WHERE post_id=p.id) as total_favourite,p.shared_post_id');
    ...
    public function get_post_detail($post_id){
        $this->db->select('p.id as post_id, p.description, p.title, p.website_url, p.location, p.latitude, p.longitude, p.views, p.link_contact_form, p.contact_email, p.status, p.shared_post_id, p.created_date, p.created_by as user_id, p.sports_id, p.sub_sports_id, sp.name as sport_name, ssp.name as sub_sports_name, u.first_name as user_first_name, u.last_name as user_last_name, u.email, u.user_group, (CASE WHEN u.user_group = "1" THEN "Admin" WHEN u.user_group = "2" THEN "Fan / User" WHEN u.user_group = "3" THEN "Coach / Trainer" WHEN u.user_group = "4" THEN "Scout / Club" WHEN u.user_group = "5" THEN "Athlete" ELSE "Admin" end ) as user_group_name, CASE WHEN u.profile_pic IS NOT NULL AND u.profile_pic != "" THEN CONCAT("'.base_url('uploads/profile/').'", u.id, "/", u.profile_pic) ELSE "" END as profile_pic, (SELECT count(*) FROM '.Tables::POST_FAVOURITE.' WHERE post_id=p.id) as total_favourite');
        $this->db->from(Tables::POSTS.' p');
        $this->db->join(Tables::SPORTS.' sp', 'p.sports_id = sp.id', 'left');
        $this->db->join(Tables::SPORTS.' ssp', 'p.sub_sports_id = ssp.id', 'left');
        $this->db->join(Tables::USER.' u', 'p.created_by = u.id', 'left');
